...
// 水面付近 Y軸減衰グラデーション
half WCE_axisYAttenMainLit(float3 WorldPos, half3 litDir, float AxisYAttenBase, float AxisYAttenCoef) {
    float signLitUnderWater = WCE_signUnderWaterMainLit(litDir);
    return saturate((AxisYAttenBase - WorldPos.y) * AxisYAttenCoef * signLitUnderWater);
}
half WCE_axisYAttenAddLit(uint idx, float3 WorldPos, float WaterSurfaceY, float AxisYAttenBase, float AxisYAttenCoef) {
...
// 深さによる減衰グラデーション
half WCE_depthAtten(float3 WorldPos, float WaterSurfaceY, float depthAttenCoef) {
    return saturate(1 - abs(WaterSurfaceY - WorldPos.y) * depthAttenCoef);
}
...
// 減衰をまとめる
half WCE_atten(half litAtten, half litShadow, half3 litDir, half dotLN, half axisYAtten, half depthAtten, half NormalAttenIntensity, half NormalAttenPowSafe, half TransparentBack) {
    ...
    return atten * litAtten * litDirAtten * axisYAtten * depthAtten;
}
...
inline half3 WCE_adjustSaturation(half3 color, half saturation) {
    ...
    return lerp(gray, color, saturation.xxx);
}
...
float WaterSurfaceY, float AxisYAttenBase, float AxisYAttenCoef, float depthAttenCoef, half IntensityMainLit,
half IntensityAddLit, float2 ColorShift, half LitSaturation, half NormalAttenIntensity, half NormalAttenPow, half TransparentBack) {
    ...
    half depthAtten = WCE_depthAtten(WorldPos, WaterSurfaceY, depthAttenCoef);
    ...
    [branch] if (IntensityMainLit > 0) {
        ...
        [branch] if (litAtten * litShadow > 0) {
            half axisYAtten = WCE_axisYAttenMainLit(WorldPos, litDir, AxisYAttenBase, AxisYAttenCoef);
            half atten = WCE_atten(litAtten, litShadow, litDir, dotLN, axisYAtten, depthAtten, NormalAttenIntensity, NormalAttenPowSafe, TransparentBack);
            [branch] if (atten > 0) {
        ...
        [branch] if (IntensityAddLit > 0) {
            ...
            [unroll(8)] for (uint i = 0u; i < pixelLitCnt; i++) {
                ...
                [branch] if (dotLN < 0 && (TransparentBack > 0 || NormalAttenIntensity < 1)) litShadow = 1;
                [branch] if (litAtten * litShadow > 0) {
                    half axisYAtten = WCE_axisYAttenAddLit(i, WorldPos, WaterSurfaceY, AxisYAttenBase, AxisYAttenCoef);
                    half atten = WCE_atten(litAtten, litShadow, litDir, dotLN, axisYAtten, depthAtten, NormalAttenIntensity, NormalAttenPowSafe, TransparentBack);
                    [branch] if (atten > 0) {
                        half3 texColor = WCE_sampleCausticsTex(WorldPos, WCE_TEX_PARAMS(CausticsTex), litDir, ScaleSafe, ColorShift, WaterSurfaceY);
                        litColor = WCE_adjustSaturation(litColor, LitSaturation);
...
// Main old (ASE用パッケージを更新してない場合に呼ばれるため残す)
half3 WCE_waterCausticsEmission(float3 WorldPos, half3 NormalWS, WCE_TEX_ARGS(CausticsTex), float Scale,
float WaterSurfaceY, float AxisYAttenBase, float AxisYAttenWidth, half IntensityMainLit, half IntensityAddLit,
float2 ColorShift, half LitSaturation, half NormalAttenIntensity, half NormalAttenPow, half TransparentBack) {
    
    float AxisYAttenCoef = 1 / max(AxisYAttenWidth, 0.000001);
    return WCE_waterCausticsEmission(WorldPos, NormalWS, WCE_TEX_PARAMS(CausticsTex), Scale,
    WaterSurfaceY, AxisYAttenBase, AxisYAttenCoef, 0, IntensityMainLit, IntensityAddLit,
    ColorShift, LitSaturation, NormalAttenIntensity, NormalAttenPow, TransparentBack);
}
