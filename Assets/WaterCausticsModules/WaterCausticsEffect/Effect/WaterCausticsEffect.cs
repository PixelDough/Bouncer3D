...
namespace MH.WaterCausticsModules {
    ...
    public class WaterCausticsEffect : MonoBehaviour {
        ...
        [SerializeField] private bool m_skipSceneView = false;
        ...
        [UnityEngine.Serialization.FormerlySerializedAs ("m_waterSurfaceAttenWide")]
        [SerializeField, Min (0f)] private float m_waterSurfaceAttenWidth = 0.5f;
        [SerializeField] private bool m_useDepthAtten = false;
        [SerializeField, Min (0f)] private float m_depthAttenDepth = 100f;

        [SerializeField, Range (0f, 10f)] private float m_intensity = 1f;
        [SerializeField, Range (0f, 10f)] private float m_adjustMainLit = 1f;
        [SerializeField, Range (0f, 10f)] private float m_adjustAddLit = 1f;
        ...
        [System.Obsolete ("This property is obsolete. Use waterSurfaceAttenWidth instead.")]
        public float waterSurfaceAttenWide {
            get => waterSurfaceAttenWidth;
            set => waterSurfaceAttenWidth = value;
        }
        public float waterSurfaceAttenWidth {
            get => m_waterSurfaceAttenWidth;
            set {
                m_waterSurfaceAttenWidth = Mathf.Max (0f, value);
                isDirty = true;
            }
        }
        public bool useDepthAtten {
            get => m_useDepthAtten;
            set {
                m_useDepthAtten = value;
                isDirty = true;
            }
        }
        public float depthAttenDepth {
            get => m_depthAttenDepth;
            set {
                m_depthAttenDepth = Mathf.Max (0f, value);
                isDirty = true;
            }
        }
        ...
        private class pID {
            ...
            readonly public static int _WCE_WaterSurfaceAttenCoef = Shader.PropertyToID ("_WCE_WaterSurfaceAttenCoef");
            readonly public static int _WCE_DepthAttenCoef = Shader.PropertyToID ("_WCE_DepthAttenCoef");
            ...
            readonly public static int _WCECF_WaterSurfaceAttenCoef = Shader.PropertyToID ("_WCECF_WaterSurfaceAttenCoef");
            readonly public static int _WCECF_DepthAttenCoef = Shader.PropertyToID ("_WCECF_DepthAttenCoef");
        ...
        private Material getMat () {
            if (_material == null)
                _material = makeMat (m_shader, "WCausticsEffect");
            return _material;
        }
        ...
        void setMaterialValue (Material mat) {
            if (mat != null) {
                ...
                mat.SetFloat (pID._WCE_WaterSurfaceAttenCoef, 1f / Mathf.Max (m_waterSurfaceAttenWidth, 0.0000001f));
                mat.SetFloat (pID._WCE_DepthAttenCoef, m_useDepthAtten ? 1f / Mathf.Max (m_depthAttenDepth, 0.0000001f) : 0f);
        ...
        private void setShaderGlobalValue () {
            ...
            Shader.SetGlobalFloat (pID._WCECF_WaterSurfaceAttenCoef, 1f / Mathf.Max (m_waterSurfaceAttenWidth, 0.0000001f));
            Shader.SetGlobalFloat (pID._WCECF_DepthAttenCoef, m_useDepthAtten ? 1f / Mathf.Max (m_depthAttenDepth, 0.0000001f) : 0f);
        ...
        private ScriptableRenderer _checkRenderer; // ScriptableRendererの変更を検知する
        ...
        void beginCameraRendering (ScriptableRenderContext context, Camera camera) {
            ...
            if (!isXRMultiPass || _checkRenderer != asset.scriptableRenderer) {
                _checkRenderer = asset.scriptableRenderer;
                enqueuePass (camera);
            }
        }
        ...
        private void enqueuePass (Camera camera) {
            ...
            if (camera.cameraType == CameraType.SceneView &&
                (m_skipSceneView || !UnityEditor.SceneView.currentDrawingSceneView.sceneLighting)) return;
            // エディタバージョンによりSceneViewのOpaqueTextureが描画されないバグ対応
            if (m_multiplyOpaqueColor && camera.cameraType == CameraType.SceneView && !asset.supportsCameraOpaqueTexture)
                enqueueOpaqueTexPass (asset);
#endif
            if (m_intensity > 0f && _pass != null)
                asset.scriptableRenderer.EnqueuePass (_pass);
        }

#if UNITY_EDITOR
        // エディタバージョンによりSceneViewのOpaqueTextureが描画されないバグ対応
        private WCEOpaqueTexPass _opaqueTexPass;
        float [] _opaqueTexScales = new float [] { 1f, 0.5f, 0.25f, 0.25f };
        private void enqueueOpaqueTexPass (UniversalRenderPipelineAsset asset) {
            if (_opaqueTexPass == null)
                _opaqueTexPass = new WCEOpaqueTexPass ();
            float scale = _opaqueTexScales [Mathf.Min ((int) asset.opaqueDownsampling, 3)];
            _opaqueTexPass.Setup (asset.scriptableRenderer, scale);
            asset.scriptableRenderer.EnqueuePass (_opaqueTexPass);
        }
#endif
...
#if UNITY_EDITOR
    // エディタバージョンによりSceneViewのOpaqueTextureが描画されないバグ対応
    internal class WCEOpaqueTexPass : ScriptableRenderPass {
        private ScriptableRenderer _scRenderer;
        private float _scale;
        RenderTargetHandle _tex;
        private Texture _storeOpaqueTex;

        public WCEOpaqueTexPass () {
            base.profilingSampler = new ProfilingSampler ("WCEOpaqueTexPass");
            this.renderPassEvent = RenderPassEvent.BeforeRenderingTransparents - 1;
            _tex.Init ("_WCEOpaqueTex");
        }

        public void Setup (ScriptableRenderer scRenderer, float scale) {
            _scRenderer = scRenderer;
            _scale = scale;
        }
        ...
        public override void Configure (CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor) {
            ...
            desc.width = (int) (desc.width * _scale);
            desc.height = (int) (desc.height * _scale);
            desc.depthBufferBits = 0;
            desc.useMipMap = false;
            desc.msaaSamples = 1;
            cmd.GetTemporaryRT (_tex.id, desc, FilterMode.Bilinear);
            ConfigureTarget (_tex.Identifier ());
        }
        public override void Execute (ScriptableRenderContext context, ref RenderingData renderingData) {
            _storeOpaqueTex = Shader.GetGlobalTexture ("_CameraOpaqueTexture");
            CommandBuffer cmd = CommandBufferPool.Get ();
            using (new ProfilingScope (cmd, base.profilingSampler)) {
                context.ExecuteCommandBuffer (cmd);
                cmd.Clear ();
                Blit (cmd, _scRenderer.cameraColorTarget, _tex.Identifier ());
                cmd.SetGlobalTexture ("_CameraOpaqueTexture", _tex.Identifier ());
            }
            context.ExecuteCommandBuffer (cmd);
            CommandBufferPool.Release (cmd);
        }
        ...
        public override void OnCameraCleanup (CommandBuffer cmd) {
            cmd.SetGlobalTexture ("_CameraOpaqueTexture", _storeOpaqueTex);
            cmd.ReleaseTemporaryRT (_tex.id);
        }
